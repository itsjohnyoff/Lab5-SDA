...
// Validates a date (checks if day, month, year are in valid ranges)
int isValidDate(int day, int month, int year) {
    ...
    if (year < 1900 || year > 2026) {
        printf("Invalid year (must be between 1900 and 2026).\n");
        return 0;
    }
    ...
    if (day < 1 || day > daysInMonth[month]) {
        printf("Invalid day for month %d (must be 1-%d).\n", month, daysInMonth[month]);
        return 0;
    }
...
// Validates gender (must be 'm' or 'f')
...
void inputCitizensManually(Citizen** registry, int count) {
    for (int i = 0; i < count; i++) {
        ...
        while (!validGender) {
            ...
            clearBuffer();
            validGender = isValidGender(registry[i]->gender);
        }
        ...
        while (!validDate) {
            ...
            clearBuffer();
            validDate = isValidDate(registry[i]->dob.day, registry[i]->dob.month, registry[i]->dob.year);
        }
...
// Calculates age accurately and assigns category and payment
void calculateAgeAndCategory(Citizen* c, Date current) {
    ...
    if (c->ageMonths < 0 || (c->ageMonths == 0 && c->ageDays < 0)) {
        c->ageYears--;
        c->ageMonths += 12;
    }
    ...
    if (c->ageDays < 0) {
        ...
        // Approximate days in previous month (using 30 as average)
        c->ageDays += 30;
    }

    // Category rules based on age in years
    if (c->ageYears < 18) {
        c->category = 1;
        c->amountPaid = 500.0;
...
int main() {
    ...
    // Ask for the current date with validation
    int validCurrentDate = 0;
    while (!validCurrentDate) {
        ...
        validCurrentDate = isValidDate(currentDate.day, currentDate.month, currentDate.year);
    }
